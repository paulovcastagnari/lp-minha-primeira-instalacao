export const transformInputText = (
  ...
  if (!!transformedText) {
    ...
    if (filter === "CHAR_ONLY" && transformedText.replaceAll) {
      transformedText = transformedText.replaceAll(
        /[^A-Za-z" "çàãáâéèêíìîóôòõúùûÇÁÃÀÂÉÈÊÍÌÎÓÕÔÒÚÙÛ]/g,
        ""
      );
    } else if (filter === "CHAR_AND_NUM" && transformedText.replaceAll) {
      transformedText = transformedText.replaceAll(
        /[^A-Za-z0-9" "çàãáâéèêíìîóôòõúùûÇÁÃÀÂÉÈÊÍÌÎÓÕÔÒÚÙÛ]/g,
        ""
      );
    } else if (filter === "CHAR_NUM_AND_UNDER" && transformedText.replaceAll) {
      transformedText = transformedText.replaceAll(
        /[^A-Za-z0-9" "çàãáâéèêíìîóôòõúùûÇÁÃÀÂÉÈÊÍÌÎÓÕÔÒÚÙÛ_]/g,
        ""
      );
    } else if (filter === "CHAR_ONLY_NO_BLANK" && transformedText.replaceAll) {
      transformedText = transformedText.replaceAll(
        /[^A-Za-zçàãáâéèêíìîóôòõúùûÇÁÃÀÂÉÈÊÍÌÎÓÕÔÒÚÙÛ]/g,
        ""
      );
    } else if (
      filter === "CHAR_AND_NUM_NO_BLANK" &&
      ...
      filter === "CHAR_AND_UNDER_NO_BLANK" &&
      transformedText.replaceAll
    ) {
      transformedText = transformedText.replaceAll(
    ...
    } else if (filter === "CONTENT" && transformedText.replaceAll) {
      transformedText = transformedText.replaceAll(
